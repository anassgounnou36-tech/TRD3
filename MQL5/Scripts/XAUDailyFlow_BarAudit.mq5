   ...
   for(int i=copied-1;i>=0;i--)
     {
      ...
      Print(StringFormat("[%s] ORW=%.2f VWAP=%.2f Regime=%s(%s) family=%d subtype=%s eligible_state=%d Selected=%d Score=%d selection_reason=%s orb_valid=%s orb_subtype=%s orb_reason=%s orb_reason_invalid=%s orb_score_raw=%d orb_score_final=%d mr_valid=%s mr_subtype=%s mr_reason=%s mr_reason_invalid=%s mr_score_raw=%d mr_score_final=%d mr_penalty=%s mr_exceptional=%s mr_block_reason=%s mr_override_reason=%s or_width_secondary_allow=%s or_primary=%.1f or_secondary=%.1f or_penalty=%d stopDistPts=%.1f targetDistPts=%.1f spreadPts=%.1f expectedSlipPts=%.1f Blocker=%s detail=%s reject=%s allow=%s",
                         TimeToString(ts,TIME_DATE|TIME_MINUTES),or_data.width,vwap.Value(),XDF_RegimeToString((int)dec.regime),dec.regime_reason,
                          ...
                          (dec.orb_signal.valid?"Y":"N"),dec.orb_subtype,dec.orb_signal.reason,dec.orb_signal.reason_invalid,dec.orb_score_raw,dec.orb_score_final,
